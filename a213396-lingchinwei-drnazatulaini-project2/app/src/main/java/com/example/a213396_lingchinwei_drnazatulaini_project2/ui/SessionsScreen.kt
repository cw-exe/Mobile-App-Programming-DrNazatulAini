...
import com.example.a213396_lingchinwei_drnazatulaini_project2.ui.theme.AccentGold
import com.example.a213396_lingchinwei_drnazatulaini_project2.ui.theme.AccentGreen
import com.example.a213396_lingchinwei_drnazatulaini_project2.ui.theme.AccentPurple
        ...
        Column(
            ...
            Box(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text(
                        text     = "SDG 4 — Every question you ask moves quality education closer.",
                        fontSize = 13.sp,
                        color    = AtlasSky
                    )
            ...
            // Quick-access row: Community Board (browse open requests) and My Responses (track sessions).
            // These are sub-screens without a bottom nav bar — back arrow returns here.
            Row(
                modifier              = Modifier
                    .fillMaxWidth()
                ...
                OutlinedButton(
                    onClick  = { navController.navigate(AtlasScreen.CommunityBoard.name) },
                    modifier = Modifier.weight(1f),
                    shape    = RoundedCornerShape(10.dp),
                    border   = BorderStroke(0.5.dp, AtlasNavy.copy(alpha = 0.3f))
                ) {
                    Icon(
                    ...
                    Text(text = "Community", fontSize = 13.sp)
                }
                OutlinedButton(
                    onClick  = { navController.navigate(AtlasScreen.MyResponses.name) },
                    modifier = Modifier.weight(1f),
                    shape    = RoundedCornerShape(10.dp),
                    border   = BorderStroke(0.5.dp, AtlasNavy.copy(alpha = 0.3f))
                ) {
                    ...
                    Spacer(modifier = Modifier.width(6.dp))
                    Text(text = "My Responses", fontSize = 13.sp)
                }
            }
            ...
            if (helpRequestList.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(helpRequestList) { request ->
                        HelpRequestCard(
                            request  = request,
                            onDelete = { viewModel.deleteHelpRequest(request) }
                        )
                    }
                }
            }
...
@Composable
fun HelpRequestCard(request: HelpRequest, onDelete: () -> Unit = {}) {
    // containerColor uses the same subject→color mapping as the chip for visual consistency
    Card(
        ...
        Column(
            ...
            // Top row: subject chip on left, badges + delete on right
            Row(
                modifier             = Modifier.fillMaxWidth(),
